...
def get_profile_ids(service):
    ...
    if accounts.get('items'):
        ...
        for account_item in accounts_items:
            ...
            batch.add(service.management().webproperties().list(accountId=account_id), callback=list_webproperties)
    ...
    if webproperties.get('items'):
        property_items = webproperties.get('items')

        for prop_item in property_items:
            prop_id = prop_item.get('id')

            profiles_batch.add(service.management().profiles().list(
                    accountId=account_id,
                    webPropertyId=prop_id).execute())

    profiles = profiles_batch.execute()
